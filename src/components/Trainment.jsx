                        ...
                        <div className="flex flex-col items-center w-full py-8 px-4 md:py-0 md:w-2/5 overflow-y-auto max-h-135">
                            <div className="flex-col flex w-full gap-4">
                                {videos?.map((video, index) => (
                                    <button
                                        className={`${video.videoId === videoId || user?.watchedVideos?.filter(watchedVideo => watchedVideo.videoId === video.videoId).map(watchedVideo => watchedVideo.videoId).includes(video.videoId) ? 'bg-cdsBlue/70' : 'bg-cdsBlue'} ${video.videoId === videoId ? 'scale-108' : ''} flex items-center p-4 min-h-20 justify-center rounded-md hover:bg-cdsBlue/70 hover:scale-105 transition-all duration-300 ease-in-out cursor-pointer`}
                                        onClick={() => {
